...
export default function ShareButton({ title, url }) {
    ...
    const items = [
        ...
        }] : []),
        {
            ...
            bgColor: copied ? '#27ae60' : '#64748b',
            icon: copied ? <Check size={18} color="#fff" /> : <Link2 size={18} color="#fff" />,
            onClick: handleCopy,
        },
        {
            key: 'whatsapp',
    ...
    return (
        <div className="share-wrapper" ref={ref}>
            <button
                className={`share-trigger ${open ? 'open' : ''}`}
                ...
                {open && (
                    ...
                    >
                        <div className="share-panel-header">
                            ...
                            <button onClick={() => setOpen(false)} className="share-panel-close" aria-label="Tutup">
                                <X size={14} />
                            </button>
                        </div>
                        <div className="share-panel-items">
                            {items.map((item) => (
                                <a
                                    key={item.key}
                                    href={item.href || '#'}
                                    ...
                                    className={`share-panel-item ${copied && item.key === 'copy' ? 'copied' : ''}`}
                                    onClick={(e) => {
                                        if (!item.href) {
                                            e.preventDefault();
                                            item.onClick();
                                ...
                                >
                                    ...
                                    <span className="share-panel-label">{item.label}</span>
                                </a>
                            ))}
                        </div>
                    </motion.div>
